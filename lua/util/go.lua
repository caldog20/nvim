    ...
    on_exit = function(_, return_val)
      vim.schedule(function()
        if return_val == 0 then
          Snacks.notify("go mod tidy completed", { id = "go-mod-tidy", title = "Go", level = vim.log.levels.INFO })
        else
          Snacks.notify("go mod tidy failed", { id = "go-mod-tidy", title = "Go", level = vim.log.levels.ERROR })
        end
      end)
    end,
  }):start()
...
function M.fill_struct_snippet()
  local bufnr = vim.api.nvim_get_current_buf()
